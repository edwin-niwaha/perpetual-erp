...
def UserTransaction(request):
    ...
    return render(
        request, "loanApp/user_transaction.html", context={"transactions": transactions}
    )
...
def UserDashboard(request):
    requestLoan = (
        loanRequest.objects.all().filter(customer=request.user.customer).count(),
    )
    approved = (
        loanRequest.objects.all()
    ...
    totalLoan = (
        CustomerLoan.objects.filter(customer=request.user.customer).aggregate(
            Sum("total_loan")
        )["total_loan__sum"],
    )
    totalPayable = (
    ...
    totalPaid = (
        loanTransaction.objects.filter(customer=request.user.customer).aggregate(
            Sum("payment")
        )["payment__sum"],
    )
